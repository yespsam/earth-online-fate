...
- 2026-06-02 content expansion: rewrote the fate descriptions, all six age-node prompts/backgrounds/facts, every node choice result, life-fragment aftermath, and ending archive copy. Expanded each age node from 3 choices to 5 choices and added `5选1` state copy plus keyboard support for number keys 1-5.
- Verified the expanded local build with `node --check app.js`, the develop-web-game Playwright client, and a full custom Playwright pass. The pass confirmed six fate cards, 5 choices per life node, key `5` selects the fifth branch, all six life nodes reach an ending, mobile 390px has no horizontal overflow, and there were no console/page errors. Screenshots saved under `/tmp/earth-online-branches-*.png`.
- TODO: none.
